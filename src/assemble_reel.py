...
def assemble_reel_white_square_video_bottom(image, video, output):
    cmd = [
        "ffmpeg",
        "-i", video,
        "-i", image,
...
def assemble_reel_white_square_video_top(image, video, output):
    cmd = [
        "ffmpeg",
        "-i", image,
        "-i", video,
        "-filter_complex",
        "[1:v]crop='min(iw,ih)':'min(iw,ih)',scale=1080:1080[vid];"
        "[0:v]scale=1080:-1[img];"
        "[img][vid]vstack=inputs=2[stacked];"
        "color=white:s=1080x1920:d=5[bg];"
        "[bg][stacked]overlay=(W-w)/2:((H-h)/2 + 70)[final]",
        "-map", "[final]",
...
def assemble_reel_blur_square_video_bottom(image, video, mask, output):
    cmd = [
        "ffmpeg",
        "-i", video,
        "-i", image,
        "-i", mask,
        "-filter_complex",
        "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,boxblur=35:1[bg];"
        "[0:v]crop='min(iw,ih)':'min(iw,ih)',scale=1080:1080[vid];"
        "[1:v]format=rgba[img];"
        "[2:v]scale=iw:ih[mask];"
        "[img][mask]alphamerge[rounded];"
